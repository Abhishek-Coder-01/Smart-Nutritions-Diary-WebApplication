...
def search():
    if request.method == "POST":
        ...
        # select all products descriptions from the database
        food = db.execute("SELECT Shrt_Desc FROM nutritions")
        ...
        if not food_arr:
            return render_template("apology.html", apology="Food not listed!")
...
def nutritions():
    if request.method == "POST":
        ...
        food_plc = db.execute(
            "SELECT Water_g, Energ_Kcal, Protein_g, Lipid_Tot_g, Carbohydrt_g, Fiber_TD_g, Sugar_Tot_g FROM nutritions WHERE Shrt_Desc = :product_single", 
            product_single=product_single
        )

        # selection food minerals from the database
        food_min = db.execute(
            "SELECT Ash_g, Calcium_mg, Iron_mg, Magnesium_mg, Phosphorus_mg, Potassium_mg, Sodium_mg, Zinc_mg, Copper_mg, Manganese_mg, Selenium_µg FROM nutritions WHERE Shrt_Desc = :product_single", 
            product_single=product_single
        )

        # selection of food vitamins from the database (three tables for easy display)
        food_vit1 = db.execute(
            "SELECT Vit_C_mg, Thiamin_mg, Riboflavin_mg, Niacin_mg, Panto_Acid_mg, Vit_B6_mg, Folate_Tot_µg, Folic_Acid_µg, Food_Folate_µg, Folate_DFE_µg, Choline_Tot_mg FROM nutritions WHERE Shrt_Desc = :product_single", 
            product_single=product_single
        )

        food_vit2 = db.execute(
            "SELECT Vit_B12_µg, Vit_A_IU, Vit_A_RAE, Retinol_µg, Alpha_Carot_µg, Beta_Carot_µg, Beta_Crypt_µg, Lycopene_µg, Lut_Zea_µg, Vit_E_mg, Vit_D_µg FROM nutritions WHERE Shrt_Desc = :product_single", 
            product_single=product_single
        )

        food_vit3 = db.execute(
            "SELECT Vit_D_IU, Vit_K_µg, FA_Sat_g, FA_Mono_g, FA_Poly_g, Cholestrl_mg FROM nutritions WHERE Shrt_Desc = :product_single", 
            product_single=product_single
        )
...
def diary_nutritions():
    if request.method == "POST":
        ...
        # if there is no quantity, redirect to the add page
        if not quantity:
            return render_template("add.html")
        ...
        db.execute("INSERT INTO history (user_id, product, quantity) VALUES (?, ?, ?)", 
                  session["user_id"], product, quantity)
...
def food_diary():
    if request.method == "GET":
        ...
        food = db.execute(
            "SELECT product, SUM(quantity) as totalFood FROM history WHERE user_id=:user_id GROUP BY product", 
        ...
        for prod in food:
            ...
            single_prod_plc = db.execute(
                "SELECT Water_g, Energ_Kcal, Protein_g, Lipid_Tot_g, Carbohydrt_g, Fiber_TD_g, Sugar_Tot_g FROM nutritions WHERE Shrt_Desc = :single_prod_name", 
                single_prod_name=single_prod_name
            )
            single_prod_min = db.execute(
                "SELECT Ash_g, Calcium_mg, Iron_mg, Magnesium_mg, Phosphorus_mg, Potassium_mg, Sodium_mg, Zinc_mg, Copper_mg, Manganese_mg, Selenium_µg FROM nutritions WHERE Shrt_Desc = :single_prod_name", 
                single_prod_name=single_prod_name
            )
            single_prod_vit1 = db.execute(
                "SELECT Vit_C_mg, Thiamin_mg, Riboflavin_mg, Niacin_mg, Panto_Acid_mg, Vit_B6_mg, Folate_Tot_µg, Folic_Acid_µg, Food_Folate_µg, Folate_DFE_µg, Choline_Tot_mg FROM nutritions WHERE Shrt_Desc = :single_prod_name", 
                single_prod_name=single_prod_name
            )
            single_prod_vit2 = db.execute(
                "SELECT Vit_B12_µg, Vit_A_IU, Vit_A_RAE, Retinol_µg, Alpha_Carot_µg, Beta_Carot_µg, Beta_Crypt_µg, Lycopene_µg, Lut_Zea_µg, Vit_E_mg, Vit_D_µg FROM nutritions WHERE Shrt_Desc = :single_prod_name", 
                single_prod_name=single_prod_name
            )
            single_prod_vit3 = db.execute(
                "SELECT Vit_D_IU, Vit_K_µg, FA_Sat_g, FA_Mono_g, FA_Poly_g, Cholestrl_mg FROM nutritions WHERE Shrt_Desc = :single_prod_name", 
                single_prod_name=single_prod_name
            )
...
def delete():
    if request.method == "GET":
        products = db.execute(
            "SELECT DISTINCT product, SUM(quantity) as totalFood FROM history WHERE user_id=:user_id GROUP BY product HAVING totalFood > 0", 
            user_id=session["user_id"]
        )
        ...
        for food in food_exist:
            if food["product"] == product:
                if int(food["totalFood"]) < int(quantity):
                    return render_template("apology.html", apology="You didn't eat so much!")

        db.execute("INSERT INTO history (user_id, product, quantity) VALUES (?, ?, ?)", 
                  session["user_id"], product, -int(quantity))
...
def errorhandler(e):
    ...
    return render_template("apology.html", apology=e.name)
